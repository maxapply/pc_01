...
((win) => {
  ...
  let initDate = (input, config) => {
    ...
    if (/^[0-9]{4}\/[0-9]{2}\/[0-9]{2}$/.test(inputval)) {
      ...
      /^[0-9]{4}\/[0-9]{2}\/[0-9]{2} [0-9]{2}:[0-9]{2}$/.test(inputval)
    ) {
      inputval = inputval + ":00"
    } else if (
      /^[0-9]{4}\/[0-9]{2}\/[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}$/.test(inputval)
    ) {} else {
      inputval = null
    }
  ...
  let createDom_Month = () => {
    let prevbtn = '<a class="date-btn-prev">&lt;</a>'
    let monthbtn = `<b class="date-btn-month" val="${cfg.month}">${
      cfg.month + 1
    }月</b>`
    let nextbtn = '<a class="date-btn-next">&gt;</a>'
  ...
  let createDom_MonthSelect = (selectedMonth) => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      mdoms += `<b class="date-option-month ${isselect}" val="${i}">${
        i + 1
      }</b>`
    }
    let dom = document.createElement("div")
  ...
  let createDom_Day = (daylist) => {
    ...
    for (var i = 0; i < data.length; i++) {
      let json = data[i]
      let daydom =
        '<b class="date-item-day${istoday}${isdayinmonth}${isselected}${isweekend}" year="${yyyy}" month="${MM}" day="${dd}">${dd}</b>'
      ...
      daydoms += daydom.replace(/\${(.+?)\}/g, function (m, key) {
  ...
  let domDay_Data = (yyyy, mm) => {
    ...
    for (let i = 0; i < 42; i++) {
      ...
      // 日是否选定(等于文本框中已选日)
      json.Isselected =
        json.yyyy == cfg.year && json.MM == cfg.month && json.dd == cfg.day
  ...
  let bindEvent_YearMonthPrevNext = () => {
    ...
    dateboxDom
      .querySelectorAll(".date-btn-prev,.date-btn-next")
      .forEach((item) => {
        item.onclick = (event) => {
          event.stopPropagation()
          let thisobj = event.currentTarget
          //
          let btntype = thisobj.parentElement.classList.contains(
              "date-area-year"
            ) ?
            1 :
            2
  ...
  let bindEvent_MinBtn = () => {
    ...
    dateboxDom.querySelector(".date-btn-minute").onclick = (event) => {
      ...
      let thisobj = event.currentTarget
      //
      let minselecct = thisobj.parentElement.querySelector(
        ".date-select-minute"
      )
  ...
  let bindEvent_SecBtn = () => {
    ...
    dateboxDom.querySelector(".date-btn-second").onclick = (event) => {
      ...
      let thisobj = event.currentTarget
      //
      let secselecct = thisobj.parentElement.querySelector(
        ".date-select-second"
      )
      // 点击时分秒下拉框按钮时,先取消其按钮的打开样式,打开后,再给自己加上打开样式
      let otherBtns = thisobj.parentElement.querySelectorAll(".date-btn-time")
  ...
  let bindEvent_DaySelected = () => {
    ...
    dateboxDom.querySelectorAll(".date-item-day").forEach((item) => {
      item.onclick = (event) => {
        ...
        //
        let date = new Date(
          getAttr(thisobj, "year"),
          getAttr(thisobj, "month"),
          getAttr(thisobj, "day"),
          cfg.hour,
          cfg.minute,
          cfg.second
  ...
  let bindEvent_OkBtn = () => {
    ...
    dateboxDom.querySelector(".date-btn-ok").onclick = (event) => {
      ...
      if (seledDay) {
        let d = new Date(
          getAttr(seledDay, "year"),
          getAttr(seledDay, "month"),
          getAttr(seledDay, "day"),
          cfg.hour,
          cfg.minute,
          cfg.second
        )
